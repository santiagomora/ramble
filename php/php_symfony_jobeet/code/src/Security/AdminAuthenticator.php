...
class AdminAuthenticator extends AbstractFormLoginAuthenticator implements PasswordAuthenticatedInterface
{
    ...
    {
        $credentials = [
            'amEmail' => $request->request->get('amEmail'),
    ...
    public function getUser($credentials, UserProviderInterface $userProvider)
    {
        ...
        $user = $this->entityManager
            ...
            ->findOneBy(['amEmail' => $credentials['amEmail']]);

        if (!$user) {
            // fail authentication with a custom error
            throw new CustomUserMessageAuthenticationException('Admin email could not be found.');
        }
        return $user;
    }
