...
import { FetchResponse, Log, LogType, Pagination } from 'src/types';
...
export class LoggingComponent implements OnInit,OnDestroy
{
  ...
  subscriptionManager: SubscriptionManager<LoggingComponent>= new SubscriptionManager<LoggingComponent>(this,loggerSubscriptions)
  ...
  fetchData = ( query = {limit:4,skip:0} ) =>
  {
    ...
    this.loggerService.fetchData(
      this.type,
      this.subscriptionManager.get<FetchResponse<Log>>('fetchPaginatedLogs'),
      {query}
    )
  }
